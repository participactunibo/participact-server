...
<script>
	...
	function updateDurationTime() {
		if (isNumber($('input[id=duration_threshold]').val())) {
			currdur = parseInt($('input[id=duration_threshold]').val())
			...
			$('#threshtimelabel').text(res);
		} else {
			$('#threshtimelabel').text("Insert a valid duration in minutes");
		}
	}
	...
	$(document).ready(function() {
		$('#duration_threshold').on('keyup keydown keypress', function(e) {
			updateDurationTime();
		});
		
		$('#duration_add_week').click(function() {
			inputAddTimeDuration('#duration_threshold', 1, "week")
		});
		$('#duration_add_day').click(function() {
			inputAddTimeDuration('#duration_threshold', 1, "day")
		});
		$('#duration_add_hour').click(function() {
			inputAddTimeDuration('#duration_threshold', 1, "hour")
		});
		$('#duration_add_minute').click(function() {
			inputAddTimeDuration('#duration_threshold', 1, "minute")
		});
		...
		<div class="row">
			...
			<div class="span9">
				<div class="page-header">
					<h1>Select an activity detection threshold for task &quot;<c:out value="${task.name}" />&quot;</h1>
				</div>
				<form:form modelAttribute="actionActivityDetection" action="${flowExecutionUrl}">
					<t:input path="duration_threshold" inputprepend="fa fa-clock-o" label="Success threshold of the activity detection in minutes." />
					<label id="threshtimelabel"></label>
					<p>
